...
## Universal quality, completeness and excellence contract

The following rules apply to every phase, work package, branch, commit, pull
request, generated artifact, release candidate and public release. They are
acceptance conditions, not aspirations:

- Every applicable requirement, ADR, control, standard, benchmark, risk, REV
  finding, legal obligation, security/privacy rule, supply-chain rule,
  organizational control and operational responsibility defined by the approved
  documentation MUST be implemented or explicitly proven not applicable. No
  scope may be silently reduced, deferred, labelled optional, hidden behind an
  MVP, replaced with a superficial answer or satisfied by a placeholder/mock.
- Every quantitative threshold MUST be declared before the work it governs,
  including its population, denominator, exclusions, tool, configuration,
  environment and evidence format. All declared benchmarks, budgets, SLOs,
  coverage, mutation, fault, recovery, compatibility and resource thresholds
  MUST be measured against the complete applicable population. Missing,
  partial, synthetic-only, skipped, retried, flaky, stale or unqualified data
  is a blocker, never a pass.
- The minimum final quality thresholds are at least 98% line coverage, 98%
  function coverage, 95% branch coverage, 100% of critical branches, 100% valid
  critical mutation coverage and 95% valid mutation coverage elsewhere, plus
  every stricter threshold defined by an area specification or phase. Critical
  catalogues and denominators MUST be complete and independently auditable;
  thresholds MUST NOT be gamed by excluding reachable code or weakening tests.
- Compatibility MUST be measured and maintained for every claimed support cell,
  including the declared Ubuntu, Windows and macOS profiles, Node/npm/toolchain
  combinations, filesystem and module formats, package consumers and external
  providers. An untested platform cell is unknown, not green; an unsupported
  cell must be explicitly declared and cannot appear in the supported matrix.
- CI, task graph, toolchain, repository organization, permissions, signing,
  DCO, branch/tag protection, dependency admission, reproducible build,
  package, SBOM, provenance, evidence-custody, audit, legal, regulatory,
  security, privacy, performance, reliability, recovery and release controls
  MUST be exercised with their positive and negative cases. A green aggregate,
  successful command, merged PR or local result is not proof of control
  acceptance by itself.
- Every claim MUST be bound to exact source/tree/edition/toolchain/configuration/
  environment identities, raw results, derived reports, digests and limitations.
  Independent or competent external evidence MUST remain independent; owner,
  tool and synthetic evidence cannot be relabelled as external or authoritative.
- Nothing is ready to merge, publish or claim until it is complete, measured,
  checked, reproducible, verified and demonstrably correct for its full scope.
  If any branch, line, mutation, benchmark, security, legal, compatibility,
  organization or operational threshold fails, the change stops, preserves the
  first failure, diagnoses affected variants, adds regression coverage and
  reruns all invalidated evidence. No retry-to-green, waiver-by-silence or
  assertion of excellence without proof is permitted.

These rules are cumulative: a later phase cannot repair an earlier phase while
leaving its evidence accepted. Any correction invalidates the affected graph
and requires a fresh exact-subject verification before the phase or release can
close.
...
## P3-B — Pre-P4 assurance gate

**Outcome:** P1–P3 are independently checked as a complete, reproducible and
measurable foundation before any P4 product implementation begins.

P3-B executes [`p3b-pre-p4-assurance.md`](p3b-pre-p4-assurance.md). It validates
