...
alta/anulacion records, immutable correction history, mode tenures and events,
installation transitions, scoped sequences, and genesis/link chain verification.
...
coverage, then mutates compiled production behavior one mutant at a time.
`gate:p4a` is the cumulative required quality task.

- 31 behavior tests cover P4-A unit, contract, security, property and fuzz cases.
