...
Every deleted-codebase finding remains adopted. P3-B assigns every row exactly
one machine-checked disposition in
`config/quality/p3b-rev-disposition.json`: 18 infrastructure findings are
`verified-prevented-in-p1-p3-scope`, while 66 fiscal-runtime, release or later
phase findings are `retained-open-not-applicable-before-p4`. The latter status
is not a pass or remediation claim; the regulatory-absence gate positively
proves why execution evidence cannot exist before P4.
