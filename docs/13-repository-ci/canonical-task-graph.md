...
Quality and closure tasks must also reject an empty or partial denominator,
unknown report status, incomplete mutation engine result, missing critical
catalogue, stale subject or mismatched production population. A task graph may
report a scoped task as `passed` only when its declared claims pass; it must not
turn a blocked phase exit into an evidence-complete phase.
