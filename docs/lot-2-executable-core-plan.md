...
That approval fixes the authoritative design input for this plan. The exact-
commit documentation and protected CI gates in `GOV-003` and `GOV-012` remain
future execution evidence; they do not make the approved design `draft` again.
Lot 2 cannot claim implementation or release evidence.
...
As of 2026-09-12, the project owner has reviewed and approved the normative
content of all 82 substantive design documents: 16 architecture, 15
public-contract, 17 format/cryptography, 16 persistence and 18 AEAT-integration
specifications. The owner has also accepted the eleven design directions
...
This completes the **normative planning content**, not implementation or formal
product release approval. Dependency admission spikes, imported digest-pinned
AEAT source bytes, generated machine registries, independent executable oracles,
negative fixtures, adapter/crash/protocol harnesses, source/package code and the
protected local/CI execution path do not yet exist. Their absence is an
implementation-state fact and does not invalidate the approved design or
accepted ADRs.
