...
const tests = [
  ...
  "tests/unit/p4-xades-policy.test.mjs",
...
const temporary = await mkdtemp(join(tmpdir(), "verifactu-p4d-coverage-"));
try {
  ...
  if (execution.error !== undefined || execution.status !== 0)
    throw new Error(
      `P4D_COVERAGE_EXECUTION: ${execution.error?.message ?? ""}\n${execution.stdout}\n${execution.stderr}`,
    );
  const output = `${execution.stdout}\n${execution.stderr}`;
  const metric = (name) => {
    const match = new RegExp(`${name}\\s*:\\s*([\\d.]+)%`, "u").exec(output);
    if (match?.[1] === undefined)
      throw new Error(`P4D_COVERAGE_METRIC: ${name}`);
    return Number(match[1]);
  };
  process.stdout.write(
    `${JSON.stringify({ schemaVersion: 1, testFiles: tests.length, statements: metric("Statements"), branches: metric("Branches"), functions: metric("Functions"), lines: metric("Lines") })}\n`,
  );
} finally {
