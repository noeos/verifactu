...
if (
  ...
  !existsSync(`${projectRoot}/packages/adapter-kit/dist/esm/index.js`)
)
  throw new Error("Build outputs are missing.");
run(process.execPath, [
  "--input-type=module",
  "--eval",
  "import('node:fs/promises').then(async ({readFile}) => { for (const f of ['packages/verifactu/package.json','packages/cli/package.json','packages/adapter-kit/package.json']) JSON.parse(await readFile(f, 'utf8')); })",
]);
